...
One important explicit limitation: all current interventions use a **whole-vector** vocabulary direction. We did **not** yet use the full channelized symbolic intervention space of the dual-stream architecture, where `x_t` can be treated head-by-head. That missing intervention family is documented in [CHANNELIZED_XT_INTERVENTION_NOTE.md](doc/notes/CHANNELIZED_XT_INTERVENTION_NOTE.md), and the concrete follow-up plan is in [PER_CHANNEL_XT_INTERVENTION_PLAN.md](doc/plans/PER_CHANNEL_XT_INTERVENTION_PLAN.md).

Implementation note:

- the repo now has a scaffolded split between
  [gpt_oss_interp/steering](gpt_oss_interp/steering),
  [gpt_oss_interp/distillation](gpt_oss_interp/distillation),
  and [gpt_oss_interp/common](gpt_oss_interp/common)
...
- [runs/direct_vocab_demo_matched_71m/report.md](runs/direct_vocab_demo_matched_71m/report.md)
