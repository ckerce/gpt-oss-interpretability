...
One important explicit limitation: all current interventions use a **whole-vector** vocabulary direction. We did **not** yet use the full channelized symbolic intervention space of the dual-stream architecture, where `x_t` can be treated head-by-head. That missing intervention family is documented in [CHANNELIZED_XT_INTERVENTION_NOTE.md](CHANNELIZED_XT_INTERVENTION_NOTE.md), and the concrete follow-up plan is in [PER_CHANNEL_XT_INTERVENTION_PLAN.md](PER_CHANNEL_XT_INTERVENTION_PLAN.md).
