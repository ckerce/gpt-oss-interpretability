...
The 9 solid threads form a four-phase argument:
...
**Phase 4 — Why does linear steering work? (Thread 14)**
Thread 14 (Bregman geometry) measures the information geometry that linear methods implicitly assume. Standard single-stream transformers have effective rank 8 in 516 dimensions — linear methods operate in 2% of the available geometry. Stream separation improves conditioning up to 22x, and a cosine diagnostic predicts which layers are safe for linear intervention. This provides the geometric foundation for why steering succeeds at some layers (threads 4–6) and why selectivity varies by task (threads 7–8).
